...
# -----------------------------------------------------------------------------
# Artifact Parsing Tests (The "Refactor Target")
# -----------------------------------------------------------------------------

@pytest.mark.parametrize("input_text, expected_type, expected_dir", [
    (
        "```markdown\n---\ntype: Design\n---\n# My Design\n```",
        "Design", "designs"
    ),
...
def test_parse_artifact_routing(repl, input_text, expected_type, expected_dir):
    """Verify correct directory routing based on YAML 'type'."""
    content, directory, filename = repl._parse_artifact(input_text)
    assert directory == expected_dir
    assert f"type: {expected_type}" in content or f"Type: {expected_type}" in content

def test_parse_artifact_nested_blocks(repl):
    """
...
"""
    ...
    assert directory == "designs"

def test_parse_artifact_fallback(repl):
    """Verify fallback when no code blocks exist but YAML does."""
    raw_text = """---
type: Intent
---
# Raw Intent
Just raw text.
"""
    content, directory, filename = repl._parse_artifact(raw_text)

    assert content == raw_text
    assert directory == "intents"
    assert filename == "raw-intent.md"

def test_parse_artifact_filename_sanitization(repl):
    """Verify filenames are kebab-cased and sanitized."""
    text = "```\n---\ntype: Design\n---\n# My Cool Feature! (v2)\n```"
    _, _, filename = repl._parse_artifact(text)

    assert filename == "my-cool-feature-v2.md"

def test_parse_artifact_default_filename(repl):
    """Verify timestamp filename if no H1 header exists."""
    text = "```\n---\ntype: Design\n---\nNo header here.\n```"
    _, _, filename = repl._parse_artifact(text)
